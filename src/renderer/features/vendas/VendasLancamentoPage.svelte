<script lang="ts">
  ...
  import type { TecidoRecord, VinculoRecord, ItemLancamentoInput, CreateVendaInput } from '../../../shared/types'

  type Props = {
    oncancel: () => void
    onsave: (venda: CreateVendaInput) => void | Promise<void>
  }
  ...
  let formaPagamento = $state('Dinheiro / PIX')
  ...
  const paymentOptions = [
    { value: 'Dinheiro / PIX', label: 'Dinheiro / PIX' },
    { value: 'Cartão de Débito', label: 'Cartão de Débito' },
    { value: 'Cartão de Crédito', label: 'Cartão de Crédito' },
    { value: 'Boleto / A Prazo', label: 'Boleto / A Prazo' }
  ]
  ...
  async function handleFinalizar() {
    ...
    try {
      ...
      erroMsg = err?.message || 'Falha ao gravar a venda.'
    } finally {
      isSaving = false
    }
  }
  ...
  <div class="tri-layout">
    ...
    <section class="col-panel col-carrinho">
      <header class="col-head">
        ...
        <span class="col-title">Lançamento de Venda</span>
      ...
      <div class="carrinho-scroll">
        {#if itensLancados.length === 0}
          <div class="carrinho-empty">
            ...
            <span class="empty-sub">Escolha o tecido e a cor ao lado, defina preço e quantidade e clique em '+ Registrar Item'.</span>
          </div>
        {:else}
      ...
      <!-- RODAPÉ FIXO COM DADOS DE PAGAMENTO E TOTAIS -->
      <footer class="col-footer-fixed">
        <div class="footer-meta-row">
          <div class="meta-field">
            <Label text="Cliente (Opcional)" for="cliente-nome" />
            <Input id="cliente-nome" bind:value={clienteNome} placeholder="Nome do cliente / balcão" />
          </div>
          <div class="meta-field">
            <Label text="Forma de Pagamento" for="forma-pagamento" />
            <Select id="forma-pagamento" bind:value={formaPagamento} options={paymentOptions} />
          </div>
        </div>
        ...
        <div class="totals-summary-bar">
          ...
          <div class="total-metric highlight">
            ...
            <span class="metric-val major">{formatCurrency(valorTotalVenda)}</span>
          </div>
        </div>
        ...
        <div class="footer-actions">
          ...
          >
            <span>{isSaving ? 'Finalizando e Imprimindo...' : 'Finalizar Venda & Imprimir (80mm)'}</span>
          </Button>
        </div>
      </footer>
    </section>
